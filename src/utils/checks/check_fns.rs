...
impl TcpdumpCheck {
    fn setup_check_watch(&self) -> anyhow::Result<pcap::PacketStream<pcap::Active, TcpdumpCodec>> {
        ...
        dbg!(&device.name);
        let capture = pcap::Capture::from_device(device)?.promisc(true);
    ...
    fn check_tcp_packet(
        ...
        if packet[30..33] == u32::from(self.ip).to_be_bytes()
            && packet[36..37] == self.port.to_be_bytes()
        {
            let offset = packet[46] as usize;
            let offset = 34 + offset * 5;

            if packet.len() - offset < self.connection_test.len() {
                None?;
            }

            if &packet[offset..] == packet {
            ...
            (packet[30..33] == u32::from(*source_addr).to_be_bytes()
                && packet[36..37] == source_port.to_be_bytes()
                && packet[26..29] == u32::from(self.ip).to_be_bytes()
                && packet[34..35] == self.port.to_be_bytes())
            .then(|| *source_port)
        }
    }
